...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        ...
        low = col_clean.lower()
        low_norm = low.replace("š", "s").replace("ć", "c").replace("č", "c")
        if low_norm == "sifra":
            rename[col] = "Sifra"
        elif low_norm == "kolicina":
            rename[col] = "Kolicina"
        elif low_norm == "stanje":
            rename[col] = "Stanje"
        elif low_norm == "datum":
            rename[col] = "Datum"
    if rename:
...
def parse_calc_excel_to_rows(
    excel_path: str, vat_default: float = 17.0, col_offsets=None
):
...
def parse_calc_df_to_rows(df, excel_path: str, vat_default: float = 17.0, col_offsets=None):
    ...
    if col_sku is None or col_name is None or col_prod_cijena_s_pdv is None:
        return False, "Fale kljucne kolone (Ident / Naziv / Prod. cijena s PDV)", []

    col_list = list(df.columns)
    offsets = col_offsets or {}

    def apply_offset(col, offset):
    ...
    col_nabavna = apply_offset(col_nabavna, offsets.get("nabavna"))
    col_prod_cijena_s_pdv = apply_offset(
        col_prod_cijena_s_pdv, offsets.get("mpc_w")
    )
    col_prod_cijena_bez_pdv = apply_offset(
        col_prod_cijena_bez_pdv, offsets.get("mpc_wo")
    )
    col_pdv_rate = apply_offset(col_pdv_rate, offsets.get("pdv_rate"))
    col_kolicina = apply_offset(col_kolicina, offsets.get("kolicina"))
...
def build_sidebar(self):
    ...
    btn3 = ctk.CTkButton(sidebar, text="Nabavka", command=self.show_proc)
    btn3.pack(pady=8, fill="x", padx=10)

    btn4 = ctk.CTkButton(sidebar, text="Podesavanja", command=self.show_settings)
...
def build_settings_page(self, parent):
    ...
    def show_kalk_review(path, rows):
        result = {"ok": False, "rows": rows, "override_skus": []}
        current_rows = list(rows)
        try:
            df_preview = pd.read_excel(path, sheet_name=0, header=None)
        except Exception:
            df_preview = None
        ...
        var_nabavna = make_offset_combo(
            "Nabavna", 1, offset_to_label(last_offsets.get("nabavna", 0))
        )
        var_mpc_wo = make_offset_combo(
            "MP bez PDV", 3, offset_to_label(last_offsets.get("mpc_wo", 0))
        )
        var_mpc_w = make_offset_combo(
            "MP s PDV", 5, offset_to_label(last_offsets.get("mpc_w", 0))
        )
        var_pdv = make_offset_combo(
            "PDV %", 7, offset_to_label(last_offsets.get("pdv_rate", 0))
        )
        var_qty = make_offset_combo(
            "Kolicina", 9, offset_to_label(last_offsets.get("kolicina", 0))
        )
        ...
        def refresh_preview(*_args):
            ...
            if df_preview is not None:
                ok, msg, rows_new = parse_calc_df_to_rows(
                    df_preview, path, col_offsets=offsets
                )
            else:
                ok, msg, rows_new = parse_calc_excel_to_rows(
                    path, col_offsets=offsets
                )
            if ok:
        ...
        for var in (var_nabavna, var_mpc_wo, var_mpc_w, var_pdv, var_qty):
        ...
        def on_confirm():
            nonlocal last_offsets
            total_rows = len(current_rows)
            bad_nab = 0
            bad_vat = 0
            for r in current_rows[: min(total_rows, 50)]:
                nab = parse_float_safe(r.get("nab_unit"))
                mpc_wo = parse_float_safe(r.get("mpc_wo_vat_unit"))
                vat = parse_float_safe(r.get("vat_rate"))
            ...
            if override_manual_var.get():
                result["override_skus"] = update_manual_overlap(current_rows)
            top.destroy()
    ...
    def refresh_kalk_prices():
        ...
        def review_cb(path, rows):
            result = show_kalk_review(path, rows)
            status = "ACCEPT" if result.get("ok") else "SKIP"
            log_entries.append(
                f"{datetime.now().isoformat(timespec='seconds')} {status} {path}"
            )
            return result

        _items, stats = build_kalk_nabavne_json_with_review(
            kalkulacije_folder,
            KALK_NABAVNE_PATH,
            review_cb,
            excel_files=candidates,
        )
        if stats.get("accepted_files", 0) == 0:
            messagebox.showinfo(
                "Info",
                "Nijedna kalkulacija nije potvrdena. "
                "Zadrzane su postojece vrijednosti.",
            )
            return
        override_skus = stats.get("override_manual_skus", [])
        if override_skus:
            removed = 0
            ...
            if removed:
                save_settings()
        for path in stats.get("accepted_paths", []):
            try:
        ...
        qty_found = stats.get("qty_found", 0)
        qty_missing = stats.get("qty_missing", 0)
        pct = (qty_found / total_rows * 100.0) if total_rows else 0.0
        messagebox.showinfo(
            "Uspjeh",
            "Azurirano nabavnih cijena iz kalkulacija: "
            f"{count} SKU.\n"
            f"Fajlovi potvrdjeni: {stats.get('accepted_files', 0)}"
...
class FemmaApp(ctk.CTk):
    def __init__(self):
        ...
        # prozor
        self.title("FEMMA 13.0 — BiH Edition (Modern UI)")
